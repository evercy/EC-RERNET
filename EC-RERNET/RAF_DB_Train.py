...
import time
import argparse
import untils_no_power as utils
#from torchstat import stat
from data.RAF import RAF
from torch.autograd import Variable
#from models import *
from models.AntCNN_fast import AntCNN
from HarDNet_cy_test import HarDNet
from CY_H_FASTERBLOCK import HarDNet
...
parser.add_argument('--dataset', type=str, default='models/RAF_no_power', help='CNN architecture')
...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.train_bs, shuffle=True, num_workers=16)

PrivateTestset = RAF(split='PrivateTest', transform=transform_test)
PrivateTestloader = torch.utils.data.DataLoader(PrivateTestset, batch_size=opt.test_bs, shuffle=False, num_workers=16)
...
net = HarDNet().cuda()
print(net)
#stat(net,(3,44,44)).cuda()
dummy_input = torch.randn(1, 3, 44, 44).cuda()
...
def PrivateTest(epoch):
    ...
    if PrivateTest_acc > best_PrivateTest_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(net, os.path.join(path, 'PrivateTest_model.pth'))
        best_PrivateTest_acc = PrivateTest_acc
        best_PrivateTest_acc_epoch = epoch
